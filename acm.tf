...
# elb様に東京リージョンでacmを発行
resource "aws_acm_certificate" "elbAcm" {
  domain_name       = var.root_domain
...
resource "aws_route53_record" "elbAcmSite" {
  for_each = {
    for acm in aws_acm_certificate.elbAcm.domain_validation_options: acm.domain_name => {
      name = acm.resource_record_name
      type = acm.resource_record_type
      record = acm.resource_record_value
    }
...
resource "aws_acm_certificate_validation" "elbAcmValid" {
  certificate_arn = aws_acm_certificate.elbAcm.arn
  validation_record_fqdns = [for record in aws_route53_record.elbAcmSite: record.fqdn]
}
